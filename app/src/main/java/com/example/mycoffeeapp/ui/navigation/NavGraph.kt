...
fun NavGraph() {
    ...
    val startDestination = if (ServiceLocator.sessionManager.isLoggedIn()) {
        NavRoutes.NavBarGraph
    } else {
        NavRoutes.WelcomeScreen
    }
    ...
    NavHost(
        ...
        composable<NavRoutes.LoginScreen> {
            LoginScreen(navControllerX, authViewModel = authViewModel)
        }

        composable<NavRoutes.SignupScreen> {
            SignupScreen(navControllerX, authViewModel = authViewModel)
        }

        composable<NavRoutes.NavBarGraph> {
//            NavBarGraph()
            NavBarGraph(rootNavController = navControllerX, authViewModel = authViewModel) // Pass the outer controller
        }
    }
}
