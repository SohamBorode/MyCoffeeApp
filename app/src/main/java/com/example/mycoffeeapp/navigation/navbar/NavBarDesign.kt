...
fun NavBarDesign(navController: NavHostController, currentRoute: String) {
    ...
    // Helper function to handle correct bottom-bar tab navigation options
     fun navigateToTab(route : Any){
        navController.navigate(route){
            ...
            launchSingleTop = true // Avoiding multiple copies of the same destination when selecting the same item again and again

            restoreState = true // Restore state(text, scroll position, list state) when re-selecting a previously selected item
        }
    }
